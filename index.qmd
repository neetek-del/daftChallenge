...
pgm.add_node("lemons", "Lemons / Citrus Intake", 0, 0, aspect=3, scale=1.2,
             plot_params={'facecolor':'#ffe066', 'edgecolor':'orange', 'linewidth':1.5})
pgm.add_node("scurvy", "Scurvy Prevention", 3, 0, aspect=3, scale=1.2,
             plot_params={'facecolor':'#b3ffb3', 'edgecolor':'green', 'linewidth':1.5})
...
pgm.add_node(
    ...
    "Acid (from Lemons)",
    0, 0,  # bottom-left
    aspect=3.5,  # fixed missing aspect
    scale=1.1,
    plot_params={
        'facecolor': '#fff44f',  # Lemon yellow
        'edgecolor': '#e6e600',  # Darker yellow
        'linewidth': 1.5
    }
)
...
pgm.add_node(
    "bacteria",
    "Bacteria Death",
    2, 1,  # slanted upwards for visual flow
    aspect=3,
    scale=1.1,
    plot_params={
        'facecolor': '#ffebcc',  # Soft orange/tan
        'edgecolor': '#cc6600',  # Darker orange/brown
        'linewidth': 1.5
    }
)
...
pgm.add_node(
    "scurvy",
    "Scurvy Prevention",
    4, 2,  # top-right for slanted line
    aspect=3,
    scale=1.1,
    plot_params={
        'facecolor': '#b3ffb3',  # Soft green
        'edgecolor': 'green',
        'linewidth': 1.5
    }
)

# Add edges for the causal chain
pgm.add_edge("acid", "bacteria")
pgm.add_edge("bacteria", "scurvy")

# Render and display
pgm.render()
plt.title("DAG 2: Misguided Belief — Acid kills Bacteria → prevents Scurvy (Slanted Line)", fontsize=12)
plt.show()
...
# Create the PGM (Probabilistic Graphical Model)
pgm = daft.PGM(dpi=150, aspect=0.8)  # Taller figure for vertical layout

# Add nodes for Vitamin C and Scurvy Prevention in a vertical line (top → bottom)
pgm.add_node(
    "vitamin_c",
    "Vitamin C Intake",
    0, 3,  # top
...
pgm.add_node(
    ...
    scale=1.2,
    plot_params={
        'facecolor': '#b3e6ff',  # Soft blue
        'edgecolor': 'navy',
        'linewidth': 2
    }
)

# Add edge showing the causal link (top → bottom)
